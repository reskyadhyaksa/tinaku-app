...
import { ArrowLeft, Save, Building2, MapPin, Navigation } from 'lucide-react';
...
export default function EditPuskesmasPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isNew) {
        await puskesmasApi.create(payload);
        toast.success('Puskesmas berhasil ditambahkan');
      } else {
        await puskesmasApi.update(params.id as string, payload);
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-pink-50">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <h2 className="text-xl font-black text-gray-900">{isNew ? 'Tambah Puskesmas Baru' : 'Edit Puskesmas'}</h2>
              <p className="text-xs md:text-sm text-gray-500 font-medium">Lengkapi detail profil puskesmas di bawah ini.</p>
            </div>
          </div>

          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <label className="text-xs font-black text-gray-700 uppercase tracking-wider block">Nama Puskesmas</label>
              <input
                required
                type="text"
                name="name"
