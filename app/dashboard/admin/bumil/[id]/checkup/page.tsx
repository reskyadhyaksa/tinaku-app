...
export default function BidanBumilCheckupPage() {
  ...
  // Collapse/Expand state for History and Form (minimized by default)
  const [isHistoryOpen, setIsHistoryOpen] = useState(false);
  const [isFormOpen, setIsFormOpen] = useState(false);

  // Automatically set active tab based on user role
  useEffect(() => {
    if (user) {
      if (user.role === 'dokter') {
        setActiveTab('doctor');
      } else {
        setActiveTab('midwife');
      }
    }
  }, [user]);
    ...
    <div className="min-h-screen bg-pink-50 p-4 md:p-8 font-sans pb-24">
      ...
      <div className="max-w-[800px] mx-auto space-y-8">
        ...
        <div className="bg-white p-6 md:px-8 md:py-6 rounded-3xl shadow-sm border border-pink-100 space-y-6">
          ...
          {/* Tab Selection Navigation - Only show if Superadmin */}
          {user?.role === 'superadmin' && (
            <div className="pt-4 border-t border-gray-50 flex justify-start">
              <div className="flex flex-col sm:flex-row bg-gray-50/50 p-1.5 rounded-[20px] border border-gray-100 w-full md:w-auto sm:min-w-[320px] gap-1">
                <button
                  onClick={() => setActiveTab('midwife')}
                  className={`flex-1 py-2.5 px-4 rounded-[14px] text-[10px] sm:text-xs font-black uppercase tracking-wider transition-all duration-200 ${
                    activeTab === 'midwife' 
        ...
        <div className="space-y-6">
          ...
          {/* 1. HISTORI PEMERIKSAAN (COLLAPSIBLE - MINIMIZED BY DEFAULT) */}
          <div className="bg-white rounded-[36px] shadow-sm border border-pink-100 overflow-hidden">
            <button
              type="button"
              onClick={() => setIsHistoryOpen(!isHistoryOpen)}
              className="w-full flex flex-col sm:flex-row sm:items-center justify-between p-5 md:p-8 gap-4 hover:bg-pink-50/10 transition-colors text-left"
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] md:text-xs text-gray-400 font-bold uppercase tracking-wider leading-relaxed">
                    {activeTab === 'midwife' ? 'Rekam Medis KIA Berkala (Bidan)' : 'Rekam USG Trimester I (Dokter)'}
                  </p>
                </div>
              </div>
              <div className="flex items-center justify-between sm:justify-end gap-3 w-full sm:w-auto border-t border-gray-50 sm:border-t-0 pt-3 sm:pt-0">
                <span className="px-3 py-1 bg-gray-50 rounded-full border border-gray-100 text-[10px] md:text-xs font-bold text-gray-500 uppercase tracking-wider">
                  {activeTab === 'midwife' ? checkups.length : doctorCheckups.length} Catatan
                </span>
            ...
            {isHistoryOpen && (
              <div className="p-6 md:p-8 border-t border-gray-50 bg-white animate-in slide-in-from-top-4 duration-300">
                {activeTab === 'midwife' ? (
          ...
          {/* 2. CATAT HASIL PEMERIKSAAN (COLLAPSIBLE - MINIMIZED BY DEFAULT) */}
          <div className="bg-white rounded-[36px] shadow-sm border border-pink-100 overflow-hidden">
            <button
              type="button"
            ...
            >
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 bg-gradient-to-br from-pink-400 to-pink-500 rounded-xl flex items-center justify-center text-white shadow-md shadow-pink-100 shrink-0">
                  <Plus className="w-5 h-5" />
                </div>
                <div>
                  <h2 className="text-base md:text-lg font-black text-gray-900">Catat Hasil Pemeriksaan</h2>
                  <p className="text-[10px] md:text-xs text-gray-400 font-bold uppercase tracking-wider leading-relaxed">
                    {activeTab === 'midwife' ? 'Formulir Catatan Bidan' : 'Formulir Rekam Medis & USG Dokter'}
                  </p>
                </div>
              </div>
              <div className="flex items-center justify-end gap-3 w-full sm:w-auto border-t border-gray-50 sm:border-t-0 pt-3 sm:pt-0">
                <span className="text-[10px] text-pink-500 font-bold uppercase tracking-wider">Isi Formulir</span>
                {isFormOpen ? <ChevronUp className="w-5 h-5 text-gray-400 animate-bounce-subtle" /> : <ChevronDown className="w-5 h-5 text-gray-400" />}
              </div>
            </button>
